...
router.get('/full', requireBranch, async (req, res) => {
  ...
  try {
    const [staff, customers, ingredients, shifts, expenses, orders, orderItems] = await Promise.all([
      ...
      db.q(`SELECT local_id, staff_id, staff_name, opening_cash, closing_cash, expected_cash,
                   variance, opened_at, closed_at, status
              FROM shifts WHERE branch_id = ? ORDER BY local_id`, [branchId]),
      db.q(`SELECT local_id, local_shift_id, staff_id, staff_name, category, description,
                   amount, from_drawer, created_at
              FROM expenses WHERE branch_id = ? ORDER BY local_id`, [branchId]),
      db.q(`SELECT local_id, total, discount, payment_method, status, cashier_name, cashier_id,
              ...
              FROM orders WHERE branch_id = ? ORDER BY local_id`, [branchId]),
      ...
      // till's own local order id, which is what a restored order_items row
      // actually needs to point at.
      db.q(`SELECT oi.local_id, o.local_id AS order_local_id, oi.menu_item_id, oi.name, oi.price,
                   oi.quantity, oi.is_deal, oi.variant_id, oi.category
              FROM order_items oi
